    ...
    <div className="page-pad max-w-[1100px] mx-auto animate-fade">
      ...
      {!analytics || analytics.studentCount === 0 ? (
        <div className="card p-12 text-center">
          ...
          <div className="text-body-md font-body-md text-surface-variant">No assessment data available yet</div>
        </div>
      ) : (
        <>
          {/* Stats */}
          <section className="responsive-grid-4 gap-3 md:gap-4 w-full mb-10 md:mb-16">
            <div className="card p-5 md:p-6">
              <div className="flex items-center gap-2 mb-3"><Users size={13} className="text-primary" /><span className="text-technical-sm font-technical-sm text-surface-variant">STUDENTS</span></div>
              <div className="text-[24px] font-technical-sm text-on-background">{analytics.studentCount}</div>
            </div>
            <div className="card p-5 md:p-6">
              <div className="flex items-center gap-2 mb-3"><BarChart3 size={13} className="text-primary" /><span className="text-technical-sm font-technical-sm text-surface-variant">AVG SCORE</span></div>
              <div className="text-[24px] font-technical-sm text-on-background">{analytics.avgUnifiedScore}</div>
            </div>
            <div className="card p-5 md:p-6">
              <div className="flex items-center gap-2 mb-3"><Target size={13} className="text-primary" /><span className="text-technical-sm font-technical-sm text-surface-variant">TOP SHAPE</span></div>
            ...
            <div className="card p-5 md:p-6">
              <div className="flex items-center gap-2 mb-3"><Users size={13} className="text-primary" /><span className="text-technical-sm font-technical-sm text-surface-variant">ASSESSMENTS</span></div>
          ...
          <section className="mb-8">
            <span className="kicker mb-4 block">Average Pillar Scores</span>
              ...
              {PILLARS.map(p => (
                <div key={p.id} className="card p-4">
                  ...
                  <div className="h-2 bg-surface-container-high mt-2 overflow-hidden">
                    <div className="h-full" style={{ width: `${analytics.avgPillarScores[p.id]}%`, background: p.color }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          {Object.keys(analytics.gradeDistribution).length > 0 && (
            <section className="mb-8">
              <span className="kicker mb-4 block">Grade Distribution</span>
              <div className="gradient-rule mb-6" />
              <div className="flex gap-3 flex-wrap">
                {Object.entries(analytics.gradeDistribution).sort().map(([grade, count]) => (
                  <div key={grade} className="card px-4 py-3 flex items-center gap-2">
                    <span className="text-label-md font-label-md text-on-background">Grade {grade}</span>
          ...
          {analytics.topPerformers.length > 0 && (
            <section className="mb-8">
              <span className="kicker mb-4 block">Top Performers</span>
              <div className="gradient-rule mb-6" />
              <div className="flex flex-col">
                {analytics.topPerformers.slice(0, 5).map((p, i) => (
                  <div key={i} className="card p-4 flex items-center gap-4 border-b-[0.5px] border-outline-variant">
