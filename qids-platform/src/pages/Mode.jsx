...
export default function Mode() {
  ...
  return <div className="min-h-screen bg-background text-on-surface">
      <header className="flex min-h-20 items-center justify-between border-b border-border px-6 lg:px-12">
        <Wordmark />
        <Link to="/" className="text-[11px] font-mono uppercase tracking-[0.15em] text-muted-foreground hover:text-on-surface no-underline transition-colors">{t("Mode.back_to_qids")}</Link>
      </header>
      <main className="mx-auto max-w-[1440px] px-6 py-16 lg:px-12 lg:py-24">
        <div className="grid grid-cols-12 gap-6">
          <div className="col-span-12 lg:col-span-4">
            <div className="label-eyebrow-gold mb-6">{t("Mode.step_01_system_context")}</div>
            <h1 className="max-w-md font-display text-[42px] leading-[1.02] tracking-[-0.03em] lg:text-[58px]">{t("Mode.where_should_development_begin")}</h1>
            <p className="mt-6 max-w-md text-[14px] leading-[1.75] text-muted-foreground">{t("Mode.the_context_adjusts_language")}</p>
          </div>
          <div className="col-span-12 lg:col-span-7 lg:col-start-6">
            <div className="border-y border-border py-5">
              <div className="flex items-center justify-between text-[10px] font-mono uppercase tracking-[0.15em] text-muted-foreground">
                <span>{t("Mode.choose_one_context")}</span>
                <span>01 / 03</span>
              </div>
              <div className="mt-6 grid grid-cols-1 gap-px border border-border bg-border md:grid-cols-2">
                ...
                return <button key={mode.id} type="button" onClick={() => setSelected(mode.id)} className={`relative bg-background p-6 text-left cursor-pointer transition-colors ${active ? 'ring-1 ring-inset ring-gold' : 'hover:bg-surface'}`}>
                      <div className="flex items-start justify-between gap-4">
                        <span className={available ? 'status-current text-gold' : 'status-future'}>
                          {mode.status}
                        </span>
                        {available ? <span className={`flex h-4 w-4 items-center justify-center border ${active ? 'border-gold bg-gold' : 'border-border-strong'}`}>
                            {active && <Check className="h-3 w-3 text-[var(--navy)]" />}
                          </span> : <LockKeyhole className="h-4 w-4 text-muted-foreground" />}
                      </div>
            ...
            <div className="flex flex-wrap items-end justify-between gap-6 border-b border-border py-8">
              <div>
                <div className="label-eyebrow mb-3">{t("Mode.selected_context")}</div>
                <div className="font-display text-[24px]">{current.name}</div>
                <div className="mt-2 text-[12px] text-muted-foreground">
                  {isAvailable ? 'Ready to open the next development step.' : 'This context is being designed and is not launchable yet.'}
                </div>
              </div>
              {isAvailable ? <button onClick={handleContinue} className="btn-primary">
                  {user ? 'Continue' : 'Create account & continue'} <ArrowRight className="h-4 w-4" />
                </button> : <span className="status-future border border-border px-4 py-3">{t("Mode.coming_soon")}</span>}
            </div>
          </div>
        </div>
        <div className="mt-24 grid grid-cols-1 gap-px border border-border bg-border md:grid-cols-3">
          <div className="bg-background p-6">
            ...
            <div className="mt-8 font-display text-[19px]">{t("Mode.one_architecture")}</div>
            <p className="mt-3 text-[12px] leading-[1.7] text-muted-foreground">{t("Mode.context_changes_the_application")}</p>
          </div>
          <div className="bg-background p-6">
            <div className="section-index">02</div>
            <div className="mt-8 font-display text-[19px]">{t("Mode.evidence_over_time")}</div>
            <p className="mt-3 text-[12px] leading-[1.7] text-muted-foreground">{t("Mode.the_assessment_is_the")}</p>
          </div>
          <div className="bg-background p-6">
            <div className="section-index">03</div>
            <div className="mt-8 font-display text-[19px]">{t("Mode.human_judgement")}</div>
            <p className="mt-3 text-[12px] leading-[1.7] text-muted-foreground">{t("Mode.the_system_supports_interpretation")}</p>
          </div>
        </div>
      </main>
    </div>;
