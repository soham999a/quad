...
function SampleRadar() {
  ...
  return <svg viewBox="0 0 300 300" width="100%" role="img" aria-label={t("Landing.sample_four_quotient_radar")}>
      {[25, 50, 75, 100].map(v => <polygon key={v} points={ring(v)} fill="none" stroke="var(--border)" strokeWidth="0.5" />)}
      {axes.map((ax, i) => {
      const [x, y] = pt(i, 100);
      ...
      {axes.map(ax => <text key={ax.key} x={ax.lx} y={ax.ly} textAnchor={ax.anchor} fill={ax.color} style={{
      font: '600 13px "Space Grotesk", sans-serif',
      letterSpacing: '0.06em'
    }}>
          {ax.key}
          <tspan fill="var(--muted-foreground, gray)" style={{
      ...
      <main>
        <section className="relative overflow-hidden border-b border-border bg-background">
          ...
          <div className="relative mx-auto grid max-w-[1440px] grid-cols-12 gap-6 px-6 pb-24 pt-20 lg:px-12 lg:pb-32 lg:pt-28">
            <div className="col-span-12 lg:col-span-6 lg:pt-8">
              ...
              <h1 className="max-w-3xl font-display text-[52px] font-light leading-[0.98] tracking-[-0.04em] md:text-[76px] lg:text-[92px]">{t("Landing.human_development")}<br />
                <span className="text-muted-foreground">{t("Landing.structured")}</span>
              </h1>
        ...
        <section className="surface-bone border-b border-border">
          <div className="mx-auto grid max-w-[1440px] grid-cols-2 gap-px bg-border px-6 lg:grid-cols-4 lg:px-12">
            {[['IV', 'Quotients measured', 'IQ · EQ · SQ · AQ'], ['03', 'Product wings', 'Individual · School · Interview'], ['∞', 'Evidence over time', 'Assessment is the entry'], ['08–60', 'Designed age range', 'Child to professional']].map(([value, label, sub]) => <div key={label} className="bg-background px-5 py-8 lg:px-8 lg:py-10">
        ...
        <section id="method" className="surface-bone border-b border-border px-6 py-24 lg:px-12 lg:py-32">
          <div className="mx-auto grid max-w-[1440px] grid-cols-12 gap-6">
            ...
            <div className="col-span-12 grid grid-cols-1 gap-px border border-border bg-border sm:grid-cols-2 lg:col-span-8">
              {QUOTIENTS.map(([index, key, title, desc, color]) => <div key={key} className="bg-background p-7 lg:p-9">
                  <div className="flex items-start justify-between">
                    <span className="section-index">{index}</span>
                    <span className="font-display text-[30px]" style={{
                  color
                }}>{key}</span>
                  </div>
        ...
        <section className="border-b border-border bg-background px-6 py-24 lg:px-12 lg:py-32">
          <div className="mx-auto max-w-[1440px]">
            ...
            <div className="grid grid-cols-1 gap-px border border-border bg-border lg:grid-cols-3">
              {WINGS.map(wing => <Link key={wing.name} to={wing.to} className="group bg-background p-8 no-underline transition-colors hover:bg-surface lg:min-h-[280px] lg:p-10">
                  <div className="flex items-start justify-between">
        ...
        <section className="surface-bone border-b border-border px-6 py-24 lg:px-12 lg:py-32">
          <div className="mx-auto grid max-w-[1440px] grid-cols-12 gap-6">
            ...
            <div className="col-span-12 grid gap-px border border-border bg-border lg:col-span-8 lg:grid-cols-2">
              {EXTENSIONS.map(ext => <Link key={ext.eyebrow} to={ext.to} className="group bg-background p-8 no-underline lg:p-10">
          ...
          <div className="mx-auto grid max-w-[1440px] grid-cols-12 gap-6">
            ...
            <div className="col-span-12 lg:col-span-8">
              <div className="divide-y divide-border border-y border-border">
                {CONTEXTS.map(([name, desc, status, to], index) => <div key={name} className="flex flex-col gap-2 py-5 md:grid md:grid-cols-12 md:items-center md:gap-4">
                    <div className="section-index md:col-span-1">0{index + 1}</div>
                    <div className="font-display text-[19px] md:col-span-4">
                      {to ? <Link to={to} className="hover:text-gold no-underline transition-colors">{name}</Link> : <span>{name}</span>}
                    </div>
        ...
        <section className="surface-bone border-b border-border px-6 py-24 lg:px-12 lg:py-32">
          <div className="mx-auto max-w-[1440px]">
            ...
            <div className="mt-16 grid grid-cols-2 gap-px border border-border bg-border md:grid-cols-4 lg:grid-cols-7">
              {LOOP.map((item, i) => <div key={item} className="bg-background p-5">
          ...
          <div className="mx-auto grid max-w-[1440px] grid-cols-12 gap-6">
            ...
            <div className="col-span-12 lg:col-span-8">
              <div className="grid gap-0 border-y border-border">
                {DIFFERENTIATORS.map(([title, desc]) => <div key={title} className="grid gap-3 border-b border-border py-6 last:border-b-0 md:grid-cols-3">
        ...
        <section className="surface-bone border-b border-border px-6 py-24 lg:px-12 lg:py-32">
          <div className="mx-auto max-w-[1440px]">
            ...
            <div className="grid grid-cols-1 gap-px border border-border bg-border md:grid-cols-3">
              ...
              features: ['Cohorts & classes', 'Evaluator console', 'School analytics', 'Priority support']
            }].map(plan => <div key={plan.name} className="bg-background p-8 lg:p-10 flex flex-col">
                  <div className="flex items-center justify-between">
                  ...
                  <ul className="mt-6 space-y-2 flex-1">
                    {plan.features.map(f => <li key={f} className="text-[13px] text-muted-foreground flex items-start gap-2">
                        <span className="text-gold mt-0.5">·</span>{f}
                      </li>)}
                  </ul>
                  {plan.status === 'CURRENT' ? <Link to="/mode" className="btn-primary mt-8 no-underline">{t('landing.cta_begin')}</Link> : <button type="button" disabled className="btn-outline mt-8 !py-3 w-full cursor-not-allowed opacity-60">{t("Landing.coming_soon")}</button>}
                </div>)}
            </div>
          </div>
        </section>
