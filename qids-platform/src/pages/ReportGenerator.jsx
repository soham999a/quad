...
export default function ReportGenerator() {
  ...
  if (!assessmentData && !hasEvalScores) {
    return <div className="page-pad max-w-[1000px] mx-auto animate-fade flex flex-col items-center justify-center min-h-[60vh] text-center">
        <h1 className="text-headline-md font-headline-md text-on-background mb-3">{t("ReportGenerator.report_generator")}</h1>
        <AlertTriangle size={28} className="text-primary mb-4 opacity-60" />
        <p className="text-label-md font-label-md text-on-surface-variant mb-6">{t("ReportGenerator.no_assessment_data_found")}</p>
        <button onClick={() => navigate('/app/assessment')} className="px-5 py-2.5 bg-primary text-on-primary text-label-md font-label-md hover:opacity-90 transition-all cursor-pointer border-none uppercase tracking-widest">{t("ReportGenerator.start_assessment")}</button>
      </div>;
  }
  const preScores = {};
  const postScores = {};
  Object.keys(PILLARS).forEach(id => {
    preScores[id] = hasEvalScores ? mergedPillarScores[id] ?? computePillarScore(id, preRaw[id] || {}) : computePillarScore(id, preRaw[id] || {});
  ...
  return <div className="page-pad max-w-[1000px] mx-auto animate-fade">
      {/* Page header */}
      <div className="flex justify-between items-start mb-8">
        <div>
          <div className="text-technical-sm font-technical-sm text-primary mb-2">{t("ReportGenerator.qids_report")}</div>
          <h1 className="text-headline-md font-headline-md text-on-background">{t("ReportGenerator.report_generator")}</h1>
        </div>
        <div className="flex gap-3">
          <button onClick={handlePrint} className="px-5 py-2.5 border-[0.5px] border-outline-variant text-on-surface-variant text-label-md font-label-md hover:text-primary hover:border-primary transition-all cursor-pointer bg-transparent uppercase tracking-widest">
            <Printer size={13} className="inline mr-2" />{t("ReportGenerator.print")}</button>
          <button onClick={handlePrint} data-tour="report-export" className="px-5 py-2.5 bg-primary text-on-primary text-label-md font-label-md hover:opacity-90 transition-all cursor-pointer border-none uppercase tracking-widest">
            <Download size={13} className="inline mr-2" />{t("ReportGenerator.export_pdf")}</button>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-8 pb-6 border-b-[0.5px] border-outline-variant">
        {[{
        id: 'full',
        label: 'Full QIDS Report'
      }, {
        id: 'profile',
        label: 'Individual Quotient Profile'
      }, {
        id: 'progress',
        label: 'Progress & Intervention Summary'
      }].map(({
        id,
        label
      }) => <button key={id} onClick={() => setReportType(id)} className={`px-5 py-2.5 text-label-md font-label-md transition-all cursor-pointer uppercase tracking-widest ${reportType === id ? 'bg-primary text-on-primary' : 'border-[0.5px] border-outline-variant text-on-surface-variant bg-transparent hover:text-primary hover:border-primary'}`}>
            {label}
          </button>)}
      </div>
      ...
      <div id="report-content" className="border-[0.5px] border-outline-variant">
        ...
        <div className="p-8 border-b-[0.5px] border-outline-variant">
          <div className="flex justify-between items-start flex-wrap gap-4">
            <div>
              <div className="text-technical-sm font-technical-sm text-primary mb-3 uppercase tracking-widest">{t("ReportGenerator.quadrant_intelligence_development_system")}</div>
              ...
              <p className="text-technical-sm font-technical-sm text-surface-variant">{t("ReportGenerator.holistic_intelligence_assessment_develop")}</p>
            </div>
            <div className="text-right">
              <div className="text-technical-sm font-technical-sm text-surface-variant mb-1">{t("ReportGenerator.context")}</div>
        ...
        <div className="p-8">
          ...
          <div className="grid grid-cols-4 gap-3 mb-8">
            ...
            label: 'Institution',
            val: intake.institution || '—'
          }, {
            label: 'Evaluator',
            val: intake.evaluator || '—'
          }, ...(hasEvalScores ? [{
            label: 'Eval Scoring',
            val: <span className="text-primary flex items-center gap-1"><UserCheck size={12} /> {evaluations.length} pillar{evaluations.length > 1 ? 's' : ''} scored</span>
          }] : [])].map(({
            label,
            val
          ...
          {/* Score summary */}
          <h3 className="text-label-md font-label-md text-on-surface mb-6 uppercase tracking-widest">{t("ReportGenerator.quotient_score_summary")}</h3>
          <div className="grid grid-cols-4 gap-3 mb-8">
            ...
            return <div key={id} className="p-4 bg-surface-container-low border-[0.5px] border-outline-variant relative">
                ...
                color: pillar.color
                  ...
                  {isIQ && <div className="text-[10px] font-technical-sm text-primary mt-1">{t("inter.normalized", { x: normalizedPost })}</div>}
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            <div className="p-6 border-[0.5px] border-primary/30 bg-primary/5">
              <div className="text-technical-sm font-technical-sm text-surface-variant mb-2 uppercase tracking-widest">{t("ReportGenerator.unified_qids_score")}</div>
            ...
            <div className="p-6 bg-surface-container-low border-[0.5px] border-outline-variant">
              <div className="text-technical-sm font-technical-sm text-surface-variant mb-4 uppercase tracking-widest">{t("ReportGenerator.weighting")}</div>
              {Object.entries(WEIGHTS).map(([k, w]) => <div key={k} className="flex justify-between mb-2 text-technical-sm font-technical-sm">
                  <span style={{
                color: PILLARS[k].color
              }}>{k} ({postScores[k]}) × {w.toFixed(2)}</span>
              ...
              <div className="border-t-[0.5px] border-outline-variant pt-3 mt-3 flex justify-between">
                <span className="text-label-md font-label-md text-on-surface">{t("ReportGenerator.unified_score")}</span>
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            <div className="p-6 bg-surface-container-low border-[0.5px] border-outline-variant">
              <h4 className="text-label-md font-label-md text-on-surface mb-4 uppercase tracking-widest">{t("ReportGenerator.quotient_radar_profile")}</h4>
              <QIDSRadar data={preScores} compare={postScores} size={240} />
            </div>
            <div className="p-6 bg-surface-container-low border-[0.5px] border-outline-variant">
              <h4 className="text-label-md font-label-md text-on-surface mb-4 uppercase tracking-widest">{t("ReportGenerator.sub_parameter_heatmap")}</h4>
              <div className="flex flex-col gap-3">
                {Object.entries(PILLARS).map(([pid, pillar]) => <div key={pid}>
                  ...
                  color: pillar.color
                    ...
                    return <div key={sp.id} className={`px-2 py-1 text-[10px] font-technical-sm font-semibold border-[0.5px] ${bg} ${borderBg} ${pct >= 75 ? 'bg-emerald-500/10' : pct >= 60 ? 'bg-amber-500/10' : 'bg-red-500/10'}`}>
                            {pct}%
                          </div>;
                  })}
                    </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            <div className="p-6 bg-surface-container-low border-[0.5px] border-outline-variant">
              <h4 className="text-label-md font-label-md text-on-surface mb-4 uppercase tracking-widest">{t("ReportGenerator.career_guidance")}</h4>
              ...
              <div className="flex flex-wrap gap-2">
                {careerProfile.roles.map(r => <span key={r} className="px-3 py-1 border-[0.5px] border-primary/30 bg-primary/5 text-technical-sm font-technical-sm text-primary">{r}</span>)}
              </div>
            </div>
            <div className="p-6 bg-surface-container-low border-[0.5px] border-outline-variant">
              <h4 className="text-label-md font-label-md text-on-surface mb-4 uppercase tracking-widest">{t("ReportGenerator.skill_shape")}</h4>
          ...
          <div className="p-6 bg-surface-container-low border-[0.5px] border-outline-variant mb-8">
            <h4 className="text-label-md font-label-md text-on-surface mb-4 uppercase tracking-widest">{t("ReportGenerator.gap_analysis_recommendations")}</h4>
            <div className="flex flex-col gap-3">
              {Object.entries(PILLARS).map(([id, pillar]) => {
              ...
              return <div key={id} className="flex items-center gap-4">
                    <span className="text-label-md font-label-md min-w-[30px]" style={{
                  color: pillar.color
                }}>{id}</span>
                    ...
                    {isCrit && <div className="flex items-center gap-1">
                        <AlertTriangle size={11} className="text-error" />
                        <span className="text-technical-sm font-technical-sm text-error">{t("ReportGenerator.critical")}</span>
                      </div>}
                  </div>;
            })}
            </div>
          ...
          <div className="p-6 border-[0.5px] border-primary/30 bg-primary/5">
            <h4 className="text-label-md font-label-md text-on-surface mb-3 uppercase tracking-widest">{t("ReportGenerator.final_recommendation")}</h4>
