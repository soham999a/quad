...
function RoleCard({
  ...
  const top = useMemo(() => (Object.entries(role.weights) as [RoleDimension, number][]).sort((a, b) => b[1] - a[1]).filter(([, v]) => v > 0).slice(0, 3), [role]);
  return <div className="card card-hover overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between gap-4 p-5 text-left cursor-pointer bg-transparent border-none touch-target">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <div className="hidden md:flex gap-1">
            {top.map(([d, v]) => <span key={d} className="chip">{DIM_WEIGHT_LABELS[d]} {v}%</span>)}
          </div>
      ...
      {open && <div className="px-5 pb-5">
          <div className="mb-4">
            <div className="kicker mb-3">{t("EmployerDashboard.competency_weight_profile")}</div>
            ...
            return <div key={d} className="flex items-center gap-3">
                  ...
                  width: `${w}%`,
                  background: `linear-gradient(90deg, ${w >= 20 ? 'var(--gold)' : w >= 10 ? 'var(--slate-muted)' : 'var(--slate-deep)'}88, ${w >= 20 ? 'var(--gold)' : w >= 10 ? 'var(--slate-muted)' : 'var(--slate-deep)'})`
                }} />
                    </div>
          ...
          <div className="flex flex-wrap gap-2">
            {RFI_THRESHOLDS.map(t => <span key={t.level} className="chip">{t.label}: {t.min}%+</span>)}
          </div>
        </div>}
    </div>;
}
...
function RfiExplorer() {
  ...
  const set = (d: RoleDimension, v: number) => setCandidate(c => ({
    ...c,
    [d]: v
  }));
  return <div>
      <p className="text-body-md text-surface-variant leading-relaxed mb-8">{t("EmployerDashboard.set_a_candidate_profile")}</p>

      <div className="grid lg:grid-cols-12 gap-8">
        <div className="lg:col-span-5">
          <div className="kicker mb-4">{t("EmployerDashboard.candidate_profile")}</div>
          <div className="flex flex-col gap-4">
            {ROLE_DIMS.map(d => <div key={d} className="card p-4">
                ...
                <input type="range" min={0} max={100} step={5} value={candidate[d]} onChange={e => set(d, Number(e.target.value))} className="w-full cursor-pointer" />
              </div>)}
          </div>
        </div>

        <div className="lg:col-span-7">
          <div className="kicker mb-4" data-tour="talent-ranking">{t("EmployerDashboard.role_fit_ranking")}</div>
          <div className="flex flex-col gap-3">
            {matches.map((r, idx) => {
            const meta = thresholdMeta(r.threshold);
            return <div key={r.roleId} className="card card-hover">
      ...
      <table className="w-full min-w-[720px] text-left">
        <thead>
          <tr className="text-technical-sm font-technical-sm text-surface-variant uppercase tracking-widest border-b-[0.5px] border-outline-variant">
            <th className="py-3 pr-4 font-normal">{t("EmployerDashboard.role")}</th>
            {ROLE_DIMS.map(d => <th key={d} className="py-3 pr-3 font-normal text-center">{d}</th>)}
            <th className="py-3 font-normal text-right">{t("EmployerDashboard.weighted")}</th>
          </tr>
        </thead>
        <tbody>
          {ROLE_PROFILES.map(r => {
          const total = Object.values(r.weights).reduce((a, b) => a + b, 0);
          return <tr key={r.id} className="border-b-[0.5px] border-outline-variant">
              ...
              return <td key={d} className="py-3 pr-3 text-center">
                      <span className="px-2.5 py-1 rounded-full text-technical-sm font-technical-sm" style={{
                  color,
...
export default function EmployerDashboard() {
  ...
  return <div className="page-pad max-w-[1520px] mx-auto animate-fade pb-24 md:pb-16">
      <section className="mb-10 md:mb-14">
        <div className="kicker mb-3">{t("EmployerDashboard.employer_intelligence")}</div>
        <h1 className="text-headline-md font-headline-md text-on-background page-headline">{t("EmployerDashboard.talent_intelligence_console")}</h1>
        <p className="text-body-md text-surface-variant max-w-2xl mt-4 leading-relaxed">{t("EmployerDashboard.browse_the_role_competency")}</p>
      ...
      <div className="flex items-center justify-between gap-6 mb-8 border-b-[0.5px] border-outline-variant">
        <div className="flex items-center gap-2">
          {TABS.map(t => <button key={t.id} onClick={() => setTab(t.id)} className={`px-5 py-2.5 rounded-full flex items-center gap-2 text-label-md font-label-md transition-all cursor-pointer border-[0.5px] touch-target ${tab === t.id ? 'bg-primary text-on-primary border-primary' : 'bg-transparent border-transparent text-surface-variant hover:text-primary'}`}>
              <t.icon size={14} /> {t.label}
            </button>)}
        </div>
        {tab === 'roles' && <div className="hidden md:flex items-center gap-2 rounded-full border-[0.5px] border-outline-variant px-4 py-2">
            <Search size={14} className="text-surface-variant" />
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder={t("EmployerDashboard.search_roles")} className="bg-transparent text-body-md text-on-background outline-none placeholder:text-surface-variant w-48" />
          </div>}
      </div>

      {tab === 'roles' && <div className="flex flex-col gap-3">
