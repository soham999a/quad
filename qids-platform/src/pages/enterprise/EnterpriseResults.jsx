...
const MODULE_LABELS = {
  CR: 'Cognitive Readiness',
  CT: 'Critical Thinking',
  SJT: 'Workplace Judgement',
  EI: 'Emotional Intelligence',
  AQ: 'Adaptability & Resilience',
  WS: 'Work Style',
  INT: 'Integrity & Risk',
  DQ: 'Decision Quality',
  LR: 'Leadership Readiness',
  ST: 'Strategic Thinking',
  PL: 'People Leadership',
  OI: 'Organisational Impact',
  RIQ: 'Role Intelligence'
};
function WorkStyleRadar({
  profile
}) {
  const data = Object.entries(profile).map(([k, v]) => ({
    subject: WS_LABELS[k] || k,
    A: v,
    fullMark: 10
  }));
...
function ModuleBars({
  ...
  const rows = Object.entries(moduleScores).filter(([, s]) => s && s.max > 0).sort((a, b) => b[1].tScore - a[1].tScore);
  return <div className="flex flex-col">
      {rows.map(([id, s], idx) => <div key={id} className="py-3 border-b-[0.5px] border-outline-variant last:border-b-0 group">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              <span className="text-technical-sm font-technical-sm" style={{
            color: BAND_COLOR[s.band] || 'var(--slate-muted)'
          }}>
          ...
          width: `${Math.max(s.tScore, 2)}%`,
          background: `linear-gradient(90deg, ${BAND_COLOR[s.band] || 'var(--phase-pre)'}88, ${BAND_COLOR[s.band] || 'var(--phase-pre)'})`
        }} />
          </div>
        </div>)}
    </div>;
...
function RfiList({
  ...
  return <div className="flex flex-col gap-3 md:gap-4">
      ...
      return <div key={r.roleId} className="card card-hover p-4 md:p-5">
            ...
            <div className="mt-4 grid grid-cols-4 md:grid-cols-8 gap-2">
              {Object.entries(r.dimensionScores).map(([d, v]) => <div key={d} className="text-center">
                ...
                width: `${v}%`,
                background: v >= 70 ? 'var(--status-ok)' : v >= 40 ? 'var(--status-warn)' : 'var(--status-err)'
              }} />
                  </div>
                </div>)}
            </div>
      ...
      {!rfi?.length && <div className="text-technical-sm font-technical-sm text-surface-variant py-6 text-center">{t("EnterpriseResults.no_role_fits_evaluated")}</div>}
    </div>;
}
export default function EnterpriseResults({
  ...
  return <div className="page-pad max-w-[1520px] mx-auto animate-fade pb-24 md:pb-16">
      <section className="mb-10 md:mb-14">
        <div className="flex items-center gap-4">
          <button onClick={onRestart} className="flex items-center gap-2 text-technical-sm font-technical-sm text-surface-variant hover:text-primary transition-colors cursor-pointer bg-transparent border-none">
            <ArrowLeft size={14} />{t("EnterpriseResults.new_assessment")}</button>
        </div>
        <div className="kicker mt-6 mb-3">{t("EnterpriseResults.performance_intelligence_profile")}</div>
        <h1 className="text-headline-md font-headline-md text-on-background page-headline">{t("EnterpriseResults.assessment_results")}</h1>
        <div className="gradient-rule mt-6" />
      </section>

      {pip && <div className="flex items-center gap-2 mb-8">
          {[{
        id: 'overview',
        label: 'Overview'
      }, {
        id: 'report',
        label: 'PIP Report'
      }].map(t => <button key={t.id} onClick={() => setView(t.id)} className={`px-5 py-2.5 rounded-full text-label-md font-label-md transition-all cursor-pointer border-[0.5px] touch-target ${view === t.id ? 'bg-primary text-on-primary border-primary' : 'bg-transparent border-outline-variant text-surface-variant hover:text-primary hover:border-primary'}`}>
              {t.label}
            </button>)}
        </div>}

      {view === 'report' && pip ? <PipReport data={pip} /> : <>
      {/* PII header */}
      {pii && <section className="relative mb-10 md:mb-14 card overflow-hidden p-6 md:p-10" style={{
        background: 'radial-gradient(ellipse at 12% 0%, var(--gold-soft), transparent 55%), linear-gradient(180deg, var(--tint-hairline), transparent 45%), var(--color-surface-container-lowest)'
      }}>
          <div aria-hidden="true" className="absolute right-0 top-0 h-full w-1/3 pointer-events-none" style={{
          background: 'radial-gradient(circle at 100% 0%, var(--gold-tint), transparent 60%)'
        }} />
          <div className="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
            <div>
              <div className="kicker mb-3">{t("EnterpriseResults.professional_intelligence_index")}</div>
            ...
            <div className="grid grid-cols-2 gap-3 min-w-[280px]">
              {agility && <div className="card p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-technical-sm font-technical-sm text-surface-variant">{t("EnterpriseResults.learning_agility")}</span>
                  </div>
              ...
              {integrity && integrityMeta && <div className="card p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="w-7 h-7 rounded-lg flex items-center justify-center" style={{
                  background: `${integrityMeta.color}22`
                }}>
                      <Shield size={14} style={{
                    color: integrityMeta.color
                  }} />
                    </span>
                    <span className="text-technical-sm font-technical-sm text-surface-variant">{t("EnterpriseResults.integrity")}</span>
                  </div>
              ...
              {result?.riq && <div className="card p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="w-7 h-7 rounded-lg flex items-center justify-center" style={{
                  background: `${BAND_COLOR[result.riq.band] || 'var(--phase-pre)'}22`
                }}>
                      <Target size={14} style={{
                    color: BAND_COLOR[result.riq.band] || 'var(--phase-pre)'
                  }} />
                    </span>
                    <span className="text-technical-sm font-technical-sm text-surface-variant">{t("EnterpriseResults.riq")}</span>
                  </div>
                  <div className="text-label-md font-label-md" style={{
                color: BAND_COLOR[result.riq.band] || 'var(--phase-pre)'
              }}>{result.riq.score} · {result.riq.band}</div>
              ...
              <div className="card p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-technical-sm font-technical-sm text-surface-variant">{t("EnterpriseResults.battery")}</span>
                </div>
                <div className="text-label-md font-label-md text-on-background">{result?.tier}</div>
                <div className="text-technical-sm font-technical-sm text-surface-variant">{t("EnterpriseResults.professional_battery")}</div>
              </div>
            </div>
          </div>
        </section>}
      ...
      <div className="grid md:grid-cols-12 gap-8 md:gap-12">
        ...
        <div className="md:col-span-7">
          <div className="flex items-center gap-3 mb-5">
            <span className="kicker">{t("EnterpriseResults.module_performance")}</span>
          </div>
        ...
        <div className="md:col-span-5">
          <div className="flex items-center gap-3 mb-5">
            <span className="kicker">{t("EnterpriseResults.work_style_profile")}</span>
          </div>
          <div className="gradient-rule mb-6" />
          {ws ? <>
              ...
              <div className="mt-4 card p-5">
                ...
                {ws.watchouts?.length > 0 && <div className="text-technical-sm font-technical-sm text-surface-variant mt-2">
                    Watch-outs: {ws.watchouts.join(', ')}
                  </div>}
              </div>
            </> : <div className="text-technical-sm font-technical-sm text-surface-variant py-6">{t("EnterpriseResults.work_style_items_not")}</div>}
        </div>
      </div>

      {/* Role Intelligence */}
      {result?.riq && <section className="mt-12 md:mt-16">
          <div className="flex items-center gap-3 mb-5">
            <span className="kicker">{t("EnterpriseResults.role_intelligence")}</span>
          </div>
          <div className="gradient-rule mb-6" />
          <div className="relative card overflow-hidden p-6 md:p-8" style={{
          background: 'radial-gradient(ellipse at 90% 0%, var(--gold-tint), transparent 55%), linear-gradient(180deg, var(--tint-hairline), transparent 45%), var(--color-surface-container-lowest)'
        }}>
            <div className="flex flex-col md:flex-row md:items-center gap-6">
              <div className="flex-1">
                <div className="text-technical-sm font-technical-sm text-surface-variant mb-1 uppercase tracking-widest">{t("inter.riq_track", { x: result.riq.track })}</div>
                <div className="text-headline-md font-headline-md text-on-background">{result.riq.trackLabel}</div>
                <p className="text-body-md text-on-surface-variant mt-2 leading-relaxed">{t("EnterpriseResults.role_specific_applied_knowledge")}</p>
              </div>
              <div className="flex items-center gap-6 flex-shrink-0">
                <div className="text-center">
                  <div className="text-[40px] leading-none font-headline-md text-gradient">{result.riq.score}</div>
                  <div className="text-technical-sm font-technical-sm text-surface-variant mt-1">{t("EnterpriseResults.t_score")}</div>
                </div>
                <div className="text-center">
                  <div className="chip text-label-md font-label-md" style={{
                  background: `${BAND_COLOR[result.riq.band]}22`,
                  color: BAND_COLOR[result.riq.band] || 'var(--phase-pre)',
                  borderColor: `${BAND_COLOR[result.riq.band] || 'var(--phase-pre)'}44`
                }}>
                    {result.riq.band}
                  </div>
            ...
            <div className="mt-5 h-[6px] rounded-full bg-surface-container-high overflow-hidden">
              ...
              width: `${Math.max(result.riq.score, 2)}%`,
              background: `linear-gradient(90deg, ${BAND_COLOR[result.riq.band] || 'var(--phase-pre)'}88, ${BAND_COLOR[result.riq.band] || 'var(--phase-pre)'})`
            }} />
            </div>
          </div>
        </section>}
      ...
      <section className="mt-12 md:mt-16">
        <div className="flex items-center gap-3 mb-5">
          <span className="kicker">{t("EnterpriseResults.role_fit_index")}</span>
        </div>
        <div className="gradient-rule mb-6" />
        <RfiList rfi={result?.rfi} />
      </section>

      {integrity?.flaggedItems?.length > 0 && <section className="mt-10 md:mt-12 rounded-xl border-[0.5px] border-error/30 bg-error/5 p-5 md:p-6">
          <div className="text-label-md font-label-md text-error mb-2">{t("EnterpriseResults.flagged_items_for_review")}</div>
