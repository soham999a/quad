...
export default function EnterpriseRunner({
  mode = 'enterprise',
  initialTier
}) {
  const {
  ...
  const start = () => {
    ...
    setStepIndex(0);
    setPhase('running');
    if (user?.uid) logEvent(user.uid, 'enterprise_started', {
      tier
    });
  };
  const onChange = (itemId, value) => setAnswers(prev => ({
    ...prev,
    [itemId]: value
  }));
  const totalItems = deployed ? deployedCount(deployed) : 0;
  const answeredItems = deployed ? Object.entries(deployed).reduce((s, [id, items]) => s + moduleAnsweredCount(items, answers), 0) : 0;
  const submit = async () => {
    ...
    clearCheckpoint(cpKind, user?.uid);
    if (!user?.uid) return;
    logEvent(user.uid, 'enterprise_complete', {
      tier
    });
    try {
      ...
      const deployedIds = Object.fromEntries(Object.entries(deployed || {}).map(([mid, items]) => [mid, items.map(i => i.id)]));
  ...
  if (phase === 'setup') {
    ...
    return <div className="page-pad max-w-[1400px] mx-auto animate-fade pb-24 md:pb-16">
        <section className="mb-12 fade-up">
          ...
          <p className="text-body-md text-surface-variant max-w-2xl mt-4 leading-relaxed">
            {mode === 'role' ? 'QGRA battery with a role-specific track generating your Role Fit Index across 8 competency dimensions.' : 'Tier-based professional intelligence assessment with adaptive difficulty, PII scoring, work style profiling and role recommendations.'}
          </p>
        ...
        <section className="mb-10 fade-up" style={{
            animationDelay: '60ms'
          }}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <span className="kicker">{t("EnterpriseRunner.select_battery")}</span>
            </div>
          ...
          {resumeState && <div className="mb-8 flex flex-col sm:flex-row sm:items-center gap-3 p-4 rounded-lg border-[0.5px] border-primary/40 bg-primary/5">
              <div className="flex-1">
                <div className="text-label-md font-label-md text-on-surface">{t("EnterpriseRunner.unfinished_session_found")}</div>
              ...
              <div className="flex gap-2 flex-shrink-0">
                <button onClick={resumeSession} className="px-4 py-2 btn-primary !py-2 !text-[12px]">{t("EnterpriseRunner.resume")}</button>
                <button onClick={discardResume} className="px-4 py-2 btn-outline !py-2 !text-[12px]">{t("EnterpriseRunner.discard")}</button>
              </div>
            </div>}

          <div className="gradient-rule mb-6" />
          <div className="grid md:grid-cols-3 gap-4">
            {ENTERPRISE_TIERS.map((tierOption, ti) => {
                
                const selected = tier === tierOption.id;
                const count = modulesForTier(tierOption.id).reduce((s, m) => s + m.deployed, 0);
                const mins = modulesForTier(tierOption.id).reduce((s, m) => s + m.timeMin, 0);
                return <button key={tierOption.id} onClick={() => setTier(tierOption.id)} aria-pressed={selected} style={{
                  animationDelay: `${120 + ti * 60}ms`
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className={`text-label-md font-label-md tracking-widest ${selected ? 'text-primary' : 'text-on-background'}`}>{tierOption.id}</div>
                    {selected && <span className="chip" style={{
                      background: 'var(--gold-soft)',
                      color: 'var(--color-primary)',
                      borderColor: 'var(--gold-line-strong)'
                    }}>
                        <Check size={12} />{t("EnterpriseRunner.selected")}</span>}
                  </div>
                  <div className="text-technical-sm font-technical-sm text-surface-variant leading-relaxed mb-4">{tierOption.desc}</div>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {modulesForTier(tier).map(m => <span key={m.id} className="chip">{m.id} <span className="text-primary/80">· {m.deployed}</span></span>)}
          </div>
        </section>
        ...
        <section className="mb-10 fade-up" style={{
          ...
          <div className="flex items-center justify-between mb-4">
            <span className="kicker">{t("EnterpriseRunner.candidate_details")}</span>
          </div>
          <div className="gradient-rule mb-6" />
          <div className="grid md:grid-cols-3 gap-5">
            <label className="flex flex-col gap-2">
              <span className="text-technical-sm font-technical-sm text-surface-variant">{t("EnterpriseRunner.full_name")}<span className="text-primary">*</span></span>
              <input value={intake.name} onChange={e => setIntake({
                  ...intake,
                  name: e.target.value
                })} placeholder={t("EnterpriseRunner.e_g_alex_morgan")} className="px-4 py-3 rounded-lg border-[0.5px] border-outline-variant bg-surface-container-lowest text-body-md text-on-background outline-none focus:border-primary placeholder:text-outline" />
            </label>
            <label className="flex flex-col gap-2">
              <span className="text-technical-sm font-technical-sm text-surface-variant">{t("EnterpriseRunner.email")}<span className="text-outline">{t("EnterpriseRunner.optional")}</span></span>
              <input value={intake.email} onChange={e => setIntake({
                  ...intake,
                  email: e.target.value
                })} placeholder={t("EnterpriseRunner.you_org_com")} type="email" autoComplete="email" className="px-4 py-3 rounded-lg border-[0.5px] border-outline-variant bg-surface-container-lowest text-body-md text-on-background outline-none focus:border-primary placeholder:text-outline" />
            </label>
            <label className="flex flex-col gap-2">
              <span className="text-technical-sm font-technical-sm text-surface-variant">{t("EnterpriseRunner.organisation")}<span className="text-outline">{t("EnterpriseRunner.optional")}</span></span>
              <input value={intake.org} onChange={e => setIntake({
                  ...intake,
                  org: e.target.value
                })} placeholder={t("EnterpriseRunner.company_institution")} className="px-4 py-3 rounded-lg border-[0.5px] border-outline-variant bg-surface-container-lowest text-body-md text-on-background outline-none focus:border-primary placeholder:text-outline" />
            </label>
          </div>
          {mode === 'role' && <div className="mt-6">
              <div className="text-technical-sm font-technical-sm text-surface-variant mb-3">{t("EnterpriseRunner.target_role")}<span className="text-outline">{t("EnterpriseRunner.optional_scopes_rfi_and")}</span></div>
              <div className="flex flex-col gap-3">
                <select value={targetRole} onChange={e => setTargetRole(e.target.value)} className="px-4 py-3 rounded-lg border-[0.5px] border-outline-variant bg-surface-container-lowest text-body-md text-on-background outline-none focus:border-primary cursor-pointer w-full md:w-1/2">
                  <option value="">{t("EnterpriseRunner.all_11_roles")}</option>
                  {ROLE_PROFILES.map(r => <option key={r.id} value={r.id}>{r.label}</option>)}
                </select>
                {roleTrack && <div className="flex items-center gap-2 w-full md:w-1/2 px-3.5 py-2.5 rounded-full border-[0.5px] border-primary/40 bg-primary/10 text-technical-sm font-technical-sm text-primary">
        ...
        <div className="fade-up flex flex-col md:flex-row md:items-center gap-5" style={{
            animationDelay: '260ms'
          }}>
          <button onClick={start} disabled={!intake.name.trim()} className="btn-primary glow w-full md:w-auto">{t("EnterpriseRunner.begin_assessment")}<ArrowRight size={14} />
          </button>
          ...
          <aside className="hidden lg:block fade-up" style={{
            ...
            <div className="sticky top-24 space-y-4">
              <div className="card p-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="kicker">{t("EnterpriseRunner.assessment_brief")}</span>
                </div>
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-technical-sm font-technical-sm text-surface-variant">{t("EnterpriseRunner.battery")}</span>
                    <span className="chip">{tier}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-technical-sm font-technical-sm text-surface-variant">{t("EnterpriseRunner.items")}</span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-technical-sm font-technical-sm text-surface-variant">{t("EnterpriseRunner.est_time")}</span>
                ...
                <div className="flex flex-wrap gap-2">
                  {modules.map(m => <span key={m.id} className="chip">
              ...
              <div className="card p-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="kicker">{t("EnterpriseRunner.you_ll_receive")}</span>
                </div>
                <ul className="space-y-3">
                  {['Professional Intelligence Index (PII)', 'Work Style Profile', 'Role Fit Index (RFI)', ...(modules.some(m => m.id === 'RIQ') ? ['Role Intelligence Quotient (RIQ)'] : [])].map(label => <li key={label} className="flex items-center gap-2.5 text-body-md text-on-background">
  ...
  const goNext = () => {
    if (stepIndex < steps.length - 1) setStepIndex(i => i + 1);else setPhase('review');
  };
  const goBack = () => {
  ...
  const header = <section className="mb-8 md:mb-10">
      <div className="flex items-center justify-between gap-4 mb-6">
        ...
        setPhase('setup');
          ...
          <ArrowLeft size={14} />{t("EnterpriseRunner.exit")}</button>
      ...
      {/* Step markers */}
      <div className="flex items-center gap-2 mb-7" aria-label={t("EnterpriseRunner.section_progress")}>
        {steps.map((step, i) => {
        ...
        return <button key={step.id} onClick={() => {
          setPhase('running');
          setStepIndex(i);
        }} aria-label={`${step.label}${done ? ' complete' : ''}`} aria-current={current ? 'step' : undefined} title={step.label} style={{
          background: done ? 'linear-gradient(90deg, var(--gold-bright), var(--gold))' : undefined
      ...
      <div className="text-body-md text-surface-variant leading-relaxed">
        {phase === 'review' ? 'Confirm your answers and submit to generate your performance intelligence profile.' : currentStep?.instructions || `${currentItems.length} items · ~${currentModule?.timeMin} min. Answer all items to continue.`}
      </div>
      <div className="gradient-rule mt-5" />
    </section>;
  if (phase === 'review') {
    return <div className="page-pad max-w-[1360px] mx-auto animate-fade pb-24 md:pb-16">
        {header}
        {allComplete && <div className="mb-6 flex items-center gap-2 px-4 py-3 rounded-full border-[0.5px] border-emerald-500/40 bg-emerald-500/10 text-technical-sm font-technical-sm text-emerald-400">
          ...
          return <button key={step.id} onClick={() => {
            ...
            setStepIndex(i);
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  <span className={`chip ${done ? '' : ''}`} style={done ? {
        ...
        <div className="mt-10 flex flex-col md:flex-row md:items-center gap-4">
          <button onClick={goBack} className="btn-outline w-full md:w-auto">
            <ArrowLeft size={14} />{t("EnterpriseRunner.back")}</button>
          <button onClick={submit} disabled={!allComplete} className="btn-primary glow w-full md:w-auto">{t("EnterpriseRunner.generate_profile")}<Check size={14} />
          </button>
          {!allComplete && <span className="text-technical-sm font-technical-sm text-amber-400/90">{t("EnterpriseRunner.answer_all_items_to")}</span>}
        </div>
      </div>;
  }
