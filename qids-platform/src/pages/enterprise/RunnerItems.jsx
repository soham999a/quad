...
function isWsAnswered(value, n) {
  return Array.isArray(value) && value.length === n && new Set(value).size === n &&
    value.every(v => typeof v === 'number' && v >= 1 && v <= n);
}

function MCQItem({ item, value, onChange, disabled }) {
  return (
    <div className="card overflow-hidden">
      ...
      <div className="flex flex-col border-t-[0.5px] border-outline-variant" role="radiogroup" aria-label={item.id}>
        {item.options.map((opt, i) => {
          ...
          return (
            <button key={i} disabled={disabled} data-opt={i} aria-pressed={selected}
              onClick={() => onChange(item.id, i)}
              className={`flex items-center gap-4 px-5 md:px-6 py-4 text-left border-b-[0.5px] border-outline-variant last:border-b-0 transition-colors cursor-pointer bg-transparent touch-target ${selected ? 'bg-primary/10' : 'hover:bg-surface-container-low'}`}>
...
function SjtItem({ item, value, onChange, disabled }) {
    ...
    <div className="card overflow-hidden">
      <div className="flex items-start gap-4 p-5 md:p-6">
        ...
        <div className="flex-1">
          ...
          <div className="mt-3 flex items-center gap-4">
            <span className="text-technical-sm font-technical-sm text-surface-variant">Mark your most effective and least effective response.</span>
          </div>
        </div>
      </div>
      <div className="flex flex-col border-t-[0.5px] border-outline-variant">
        <div className="hidden md:flex items-center px-6 py-2 border-b-[0.5px] border-outline-variant">
          ...
          <span className="w-16 text-center text-technical-sm font-technical-sm text-surface-variant">MOST</span>
          <span className="w-16 text-center text-technical-sm font-technical-sm text-surface-variant">LEAST</span>
        </div>
        {item.options.map((opt, i) => {
          const isMost = most === i;
          ...
          return (
            <div key={i}
              className={`flex items-center gap-4 px-5 md:px-6 py-4 border-b-[0.5px] border-outline-variant last:border-b-0 ${isMost || isLeast ? 'bg-primary/5' : ''}`}>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button disabled={disabled}
                  ...
                  aria-label={`Mark option ${LETTERS[i]} as most effective`}
                  data-opt-most={i}
                  className={`flex items-center justify-center gap-1 px-3 py-2 rounded-md text-technical-sm font-technical-sm border-[0.5px] cursor-pointer touch-target transition-colors ${isMost ? 'bg-emerald-500 text-white border-emerald-500' : 'border-outline-variant text-surface-variant hover:text-emerald-500 hover:border-emerald-500 bg-transparent'}`}>
                  <ArrowUp size={12} />
                ...
                <button disabled={disabled}
                  ...
                  aria-label={`Mark option ${LETTERS[i]} as least effective`}
                  data-opt-least={i}
                  className={`flex items-center justify-center gap-1 px-3 py-2 rounded-md text-technical-sm font-technical-sm border-[0.5px] cursor-pointer touch-target transition-colors ${isLeast ? 'bg-rose-500 text-white border-rose-500' : 'border-outline-variant text-surface-variant hover:text-rose-500 hover:border-rose-500 bg-transparent'}`}>
                  <ArrowDown size={12} />
...
function WsItem({ item, value, onChange, disabled }) {
  ...
  return (
    <div className="card overflow-hidden">
      <div className="flex items-start gap-4 p-5 md:p-6">
        ...
        <div className="flex-1">
          <p className="text-body-md text-on-background leading-relaxed">{item.stem}</p>
          <div className="mt-3 text-technical-sm font-technical-sm text-surface-variant">
            Rank each statement — 4 = most like you, 1 = least like you. Each rank used once.
          </div>
        </div>
      </div>
      <div className="flex flex-col border-t-[0.5px] border-outline-variant">
        <div className="hidden md:flex items-center px-6 py-2 border-b-[0.5px] border-outline-variant">
          <span className="flex-1" />
          {[4, 3, 2, 1].map(r => (
            <span key={r} className="w-12 text-center text-technical-sm font-technical-sm text-surface-variant">{r === 4 ? 'MOST' : r === 1 ? 'LEAST' : r}</span>
          ))}
        </div>
        {item.options.map((opt, i) => (
          <div key={i} className="flex items-center gap-4 px-5 md:px-6 py-4 border-b-[0.5px] border-outline-variant last:border-b-0">
            <span className="text-body-md text-on-surface-variant flex-1">{opt}</span>
            <div className="flex items-center gap-2 flex-shrink-0">
              {[4, 3, 2, 1].map(r => {
                ...
                return (
                  <button key={r} disabled={disabled}
                    ...
                    aria-label={`Rank option ${LETTERS[i]} as ${r}`}
                    data-rank-opt={i} data-rank={r}
...
export default function RunnerItems({ module, items, answers, onChange, disabled }) {
  return (
    <div className="flex flex-col gap-6 md:gap-8">
      {items.map((item, idx) => {
        const value = answers[item.id];
        const isAnswered =
          module.scoring === 'best_worst' ? isSjtAnswered(value) :
          module.scoring === 'ipsative' ? isWsAnswered(value, item.options.length) :
          isMcqAnswered(value);
        ...
        return (
          <div key={item.id} data-item={item.id}>
            <div className="flex items-center gap-2.5 mb-2.5 px-1">
              ...
              {isAnswered && (
                <span className="text-technical-sm font-technical-sm text-emerald-500 ml-auto flex items-center gap-1 flex-shrink-0">
                  <Check size={11} /> Answered
                </span>
              )}
            </div>
            {module.scoring === 'best_worst' ? (
              <SjtItem item={item} value={value} onChange={onChange} disabled={disabled} />
            ) : module.scoring === 'ipsative' ? (
              <WsItem item={item} value={value} onChange={onChange} disabled={disabled} />
            ) : (
              <MCQItem item={item} value={value} onChange={onChange} disabled={disabled} />
            )}
          </div>
        );
      })}
