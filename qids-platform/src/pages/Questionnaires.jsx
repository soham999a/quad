    ...
    <div>
      ...
      <div className="mt-3.5">
        {compData[activeComp].questions[ageGroup].map((q, i) => (
          <div key={i} className="mb-3 px-3.5 py-3 rounded-xl border border-outline-variant" style={{ background: 'var(--tint-hairline)' }}>
        ...
        <div>
          ...
          {SQ_QUESTIONS.component1_ACE.exercises.map(ex => (
            <div key={ex.id} className="mb-4 bg-surface-container-low rounded-xl p-4" style={{ border: `1px solid ${color}20` }}>
              ...
              {ex.rubric.map(r => (
                <div key={r.criterion} className="px-2.5 py-2 mb-1 rounded-md border border-outline-variant text-xs" style={{ background: 'var(--tint-subtle)' }}>
        ...
        <div>
          ...
          {SQ_QUESTIONS.component2_CSI.questions.map((q, qi) => (
            <div key={q.id} className="mb-4 bg-surface-container-low border border-outline-variant rounded-xl p-4">
              ...
              <div className="text-xs text-on-surface-variant leading-relaxed mb-2 px-2.5 py-2 rounded-md" style={{ background: 'var(--tint-subtle)', borderLeft: `3px solid ${color}40` }}><strong>Scenario:</strong> {q.scenario}</div>
              ...
              {q.options.map((opt, oi) => {
                ...
                return (
                  <div key={oi} className="px-3 py-[7px] mb-1 rounded-lg border border-outline-variant text-xs flex justify-between items-center" style={{ background: 'var(--tint-hairline)' }}>
...
function MCQInput({ options, answer, question, index }) {
  ...
  return (
    <div className="mb-3.5 px-3.5 py-3 rounded-xl border border-outline-variant" style={{ background: 'var(--tint-hairline)' }}>
...
function OpenInput({ question, index }) {
  return (
    <div className="mb-3 px-3.5 py-3 rounded-xl border border-outline-variant" style={{ background: 'var(--tint-hairline)' }}>
      {question && <div className="text-[13px] font-medium mb-2 leading-normal"><span className="text-surface-variant mr-1.5">{(index ?? 0) + 1}.</span>{question}</div>}
      <textarea placeholder="Write your answer here..." className="w-full px-2.5 py-2 rounded-lg text-xs text-on-surface leading-normal resize-y min-h-[56px] box-border"
        style={{ background: 'var(--tint-subtle)', border: '1px solid var(--border-outline-variant)', fontFamily: 'Sora' }} />
    </div>
  );
}
