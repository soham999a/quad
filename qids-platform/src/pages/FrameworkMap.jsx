...
const FRAMEWORK_TREE = {
  root: { label: 'Quadrant Intelligence Development System (QIDS)', color: 'var(--phase-pre)' },
  branches: [
    {
      id: 'pillars', label: 'The Four Pillars', color: 'var(--phase-pre)',
      children: [
        { id: 'iq', label: 'Intelligence Quotient (IQ)', color: 'var(--phase-pre)', children: ['Verbal Intelligence', 'Quantitative Intelligence', 'Psychometric Abilities', 'Performance Intelligence'] },
        { id: 'eq', label: 'Emotional Quotient (EQ)', color: 'var(--status-ok)', children: ['Self-Awareness', 'Self-Management', 'Social Awareness', 'Relationship Management', 'Emotional Resilience'] },
        { id: 'sq', label: 'Social Quotient (SQ)', color: 'var(--phase-int)', children: ['Assessment Center Exercises', 'Cognitive Social Intelligence', 'Performance-Based Activities'] },
        { id: 'aq', label: 'Adversity Quotient (AQ)', color: 'var(--status-warn)', children: ['Situational Agility', 'Proactive Momentum', 'Adversity Response', 'Growth Integration', 'Resilience Sustainability'] },
      ],
    },
    {
      id: 'assessment', label: 'Assessment Architecture', color: 'var(--cyan-mid)',
      children: [
        { id: 'phase1', label: 'Phase 1: Pre-Intervention', color: 'var(--phase-pre)', children: ['Baseline Measurement', 'Banding (Red/Yellow/Green)'] },
        { id: 'phase2', label: 'Phase 2: Strategic Intervention', color: 'var(--phase-int)', children: ['Targeted Training Modules', 'Counselling & Meetings'] },
        { id: 'phase3', label: 'Phase 3: Post-Intervention', color: 'var(--phase-post)', children: ['Progress Evaluation', 'Future Planning'] },
      ],
    },
    {
      id: 'methodologies', label: 'Methodologies & Tools', color: 'var(--phase-int)',
      children: [
        { id: 'dec', label: 'Dynamic Emotional Competency (DEC)', color: 'var(--status-ok)', children: [] },
        { id: 'rdf', label: 'Resilience Dynamics Framework', color: 'var(--status-warn)', children: [] },
        { id: 'dwa', label: 'Dynamic Weightage Algorithm', color: 'var(--phase-pre)', children: [] },
        { id: 'sim', label: 'Simulations & Role-play', color: 'var(--phase-int)', children: [] },
      ],
    },
    {
      id: 'goals', label: 'Strategic Goals', color: 'var(--phase-post)',
      children: [
        { id: 'g1', label: 'Holistic Personality Development', color: 'var(--phase-post)', children: [] },
        { id: 'g2', label: 'Career Guidance Alignment', color: 'var(--cyan-mid)', children: [] },
        { id: 'g3', label: 'Educational System Reform', color: 'var(--phase-pre)', children: [] },
        { id: 'g4', label: 'Man-making Education', color: 'var(--phase-int)', children: [] },
      ],
    },
    {
      id: 'grading', label: 'Grading Mechanism', color: 'var(--status-warn)',
      children: [
        { id: 'ga', label: 'A: Excellent (90–100%)', color: 'var(--status-ok)', children: [] },
        { id: 'gb', label: 'B: Very Good (75–89%)', color: 'var(--cyan-mid)', children: [] },
        { id: 'gc', label: 'C: Good (60–74%)', color: 'var(--status-warn)', children: [] },
        { id: 'gd', label: 'D: Satisfactory (45–59%)', color: 'var(--orange-mid)', children: [] },
        { id: 'ge', label: 'E: Needs Improvement (<45%)', color: 'var(--status-err)', children: [] },
      ],
    },
  ],
};
...
function TreeNode({ node, depth = 0, expanded, onToggle, onSelect, selected }) {
  ...
  const handleKey = (e) => {
    if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); if (hasChildren) onToggle(node.id); onSelect(node); }
  };
  ...
  return (
    <div style={{ marginLeft: depth * 24 }}>
      <div
        ...
        className="flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer mb-1 transition-all focus-visible:outline focus-visible:outline-gold"
        style={{
      ...
      {hasChildren && isExpanded && (
        <div style={{ borderLeft: `1px solid ${alpha(node.color, 30)}`, marginLeft: 16, paddingLeft: 8 }}>
      ...
      <div className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="flex flex-col gap-3">
          {FRAMEWORK_TREE.branches.map(branch => (
            <div key={branch.id}
              role="button"
              tabIndex={0}
              aria-expanded={!!expanded[branch.id]}
              onKeyDown={e => onKey(e, () => { toggle(branch.id); select(branch); })}
              className="rounded-xl p-4 cursor-pointer transition-all border-l-[3px] focus-visible:outline focus-visible:outline-gold"
              style={{ background: 'var(--navy-4)', borderColor: alpha(branch.color, 30), borderLeftColor: branch.color }}
              onClick={() => { toggle(branch.id); select(branch); }}
              onMouseEnter={e => e.currentTarget.style.borderColor = branch.color}
              onMouseLeave={e => e.currentTarget.style.borderColor = alpha(branch.color, 30)}
            >
                ...
                <div className="flex flex-wrap gap-2 pl-5">
                  {branch.children.map(child => (
                    <div key={child.id} onClick={e => { e.stopPropagation(); select(child); }}
                      onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); select(child); } }}
                      role="button"
                      tabIndex={0}
                      className="px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer transition-all focus-visible:outline focus-visible:outline-gold"
                      style={{ background: alpha(child.color, 15), border: `1px solid ${alpha(child.color, 40)}`, color: child.color }}
                      onMouseEnter={e => e.currentTarget.style.background = alpha(child.color, 25)}
                      onMouseLeave={e => e.currentTarget.style.background = alpha(child.color, 15)}
                    >
                      {child.label}
                    </div>
                  ))}
        ...
        <div className="mt-7">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              { g: 'A', label: 'Excellent', range: '90–100', color: 'var(--status-ok)' },
              { g: 'B', label: 'Very Good', range: '75–89', color: 'var(--cyan-mid)' },
              { g: 'C', label: 'Good', range: '60–74', color: 'var(--status-warn)' },
              { g: 'D', label: 'Satisfactory', range: '45–59', color: 'var(--orange-mid)' },
              { g: 'E', label: 'Needs Improvement', range: '<45', color: 'var(--status-err)' },
            ].map(({ g, label, range, color }) => (
              <div key={g} className="flex-1 px-2.5 py-3 rounded-lg text-center" style={{ background: alpha(color, 15), border: `1px solid ${alpha(color, 40)}` }}>
        ...
        <div className="mt-6 p-5 rounded-xl" style={{ background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.2)' }}>
          ...
          <div className="mt-3 flex gap-4 flex-wrap">
            {[['IQ', '×1.00', 'var(--phase-pre)'], ['EQ', '×2.00', 'var(--status-ok)'], ['SQ', '×2.00', 'var(--phase-int)'], ['AQ', '×1.28', 'var(--status-warn)']].map(([k, w, c]) => (
