...
const CONTEXTS = [{
  ...
  label: 'Individual',
  icon: Sparkles,
  desc: 'Personal growth and a longitudinal capability baseline.'
}, {
  id: 'school',
  label: 'School / College',
  icon: GraduationCap,
  desc: 'Classrooms, cohorts, and age-banded development.'
}, {
  id: 'corporate',
  label: 'Corporate',
  icon: Building2,
  desc: 'Teams, leaders, and evidence-based hiring.'
}, {
  id: 'interview',
  label: 'Interview',
  icon: Microscope,
  desc: 'Structured, evidence-based interview intelligence.'
}];
const PERSONAS = {
  individual: [{
    id: 'individual',
    label: 'Individual',
    icon: Sparkles,
    desc: 'Personal development journey'
  }, {
    id: 'student',
    label: 'Student',
    icon: GraduationCap,
    desc: 'School or college learner'
  }],
  school: [{
    id: 'teacher',
    label: 'Teacher',
    ...
    label: 'Student',
    icon: GraduationCap,
    desc: 'Join a class and develop'
  }, {
    id: 'evaluator',
    label: 'Evaluator',
    icon: Microscope,
    desc: 'Assess and guide others'
  }],
  corporate: [{
    id: 'employer',
    label: 'Employer',
    icon: Building2,
    desc: 'Hiring and talent intelligence'
  }, {
    id: 'individual',
    label: 'Individual',
    icon: Sparkles,
  ...
  interview: [{
    ...
    label: 'Interviewer',
    icon: Microscope,
    desc: 'Run structured interviews'
  }, {
    id: 'individual',
    label: 'Candidate',
    icon: Target,
    desc: 'Prepare evidence of readiness'
  }]
};
const GOALS = {
  default: [{
    ...
    icon: Briefcase,
    desc: 'Which roles and paths match my profile.'
  }, {
    id: 'growth',
    label: 'Personal growth',
    icon: Sparkles,
    desc: 'Understand strengths and development areas.'
  }, {
    id: 'academic',
    label: 'Academic guidance',
    icon: GraduationCap,
    desc: 'Learning styles and academic potential.'
  }, {
    id: 'general',
    label: 'General exploration',
    icon: Target,
    desc: 'A complete intelligence blueprint.'
  }],
  teacher: [{
    id: 'cohort',
    label: 'Run a cohort',
    icon: Users,
    desc: 'Establish baselines and track growth.'
  }, {
    id: 'monitor',
    label: 'Monitor progress',
    icon: Microscope,
    desc: 'Watch class-level analytics over time.'
  }],
  employer: [{
    id: 'deploy',
    label: 'Deploy a battery',
    icon: Building2,
    desc: 'Assess candidates against a tier + role.'
  }, {
    id: 'benchmark',
    label: 'Benchmark talent',
    icon: Target,
    desc: 'Compare candidates and cohort insight.'
  }],
  evaluator: [{
    id: 'scoring',
    label: 'Score assessments',
    icon: Check,
    desc: 'Rubric and evaluator scoring.'
  }, {
    id: 'interviews',
    label: 'Run interviews',
    icon: Microscope,
    desc: 'Structured interview studio.'
  }]
};
const PLANS = [{
  id: 'free',
...
function ChoiceGrid({
  ...
  return <div className="grid md:grid-cols-2 gap-3">
      ...
      return <button key={opt.id} type="button" onClick={() => onSelect(opt.id)} className={`relative text-left p-5 card card-hover cursor-pointer transition-all duration-200 ${active ? 'card-gold' : ''}`}>
            <div className="flex items-start justify-between gap-4">
              {Icon && <span className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${active ? 'bg-gold-soft' : 'bg-surface-container-high'}`}>
...
function Stepper({
  ...
  return <div className="flex items-center gap-2 overflow-x-auto pb-1">
      ...
      return <div key={label} className="flex items-center gap-2 shrink-0">
            <div className="flex items-center gap-2">
              <span className={`h-5 w-5 rounded-sm border flex items-center justify-center font-mono text-[10px] ${state === 'done' ? 'border-gold bg-gold text-[var(--navy)]' : state === 'active' ? 'border-gold text-gold' : 'border-border-strong text-muted-foreground'}`}>
...
const STEP_VIEWS = (t) => ({
  context: (s, set) => <>
      <FieldHeading step={0} title={<>{t("Onboarding.where_should_development")}<span className="text-muted-foreground">{t("Onboarding.begin")}</span></>} />
      <p className="mb-8 max-w-xl text-[14px] leading-[1.75] text-muted-foreground">{t("Onboarding.the_context_adjusts_language")}</p>
      <ChoiceGrid options={CONTEXTS} selected={s.contextId} onSelect={id => set({
      ...s,
      contextId: id,
      persona: '',
      goalId: ''
  ...
  role: (s, set) => <>
      <FieldHeading step={1} title={<>{t("Onboarding.who_are")}<span className="text-muted-foreground">{t("Onboarding.you")}</span></>} />
      <p className="mb-8 max-w-xl text-[14px] leading-[1.75] text-muted-foreground">{t("Onboarding.your_role_decides_which")}</p>
      <ChoiceGrid options={personasFor(s.contextId)} selected={s.persona} onSelect={id => set({
      ...s,
      persona: id,
      goalId: ''
    })} />
  ...
  goal: (s, set) => <>
      <FieldHeading step={2} title={<>{t("Onboarding.what_brings")}<span className="text-muted-foreground">{t("Onboarding.you_here")}</span></>} />
      <p className="mb-8 max-w-xl text-[14px] leading-[1.75] text-muted-foreground">{t("Onboarding.we_ll_tailor_your")}</p>
      <ChoiceGrid options={goalsFor(s.persona)} selected={s.goalId} onSelect={id => set({
      ...s,
      goalId: id
    })} />
    </>,
  profile: (s, set) => <>
      <FieldHeading step={3} title={<>{t("Onboarding.fine_tune_your")}<span className="text-muted-foreground">{t("Onboarding.baseline")}</span></>} />
      <p className="mb-8 max-w-xl text-[14px] leading-[1.75] text-muted-foreground">{t("Onboarding.a_couple_of_quick")}</p>
      {s.persona === 'individual' || s.persona === 'student' ? <div className="space-y-8">
          <div>
            <div className="kicker mb-4">{t("Onboarding.age_group")}</div>
            <div className="grid md:grid-cols-2 gap-3">
              {[{
            id: '11-18',
            label: '11–18',
            desc: 'School / College student',
            icon: GraduationCap
          }, {
            id: '19-32',
            label: '19–32',
            desc: 'Young professional',
            icon: Briefcase
          }].map(ag => <button key={ag.id} type="button" onClick={() => set({
            ...s,
            ageGroup: ag.id
          ...
          <div>
            <div className="kicker mb-4">{t("Onboarding.assessment_purpose")}</div>
            <div className="grid md:grid-cols-2 gap-3">
              {[{
            id: 'career',
            label: 'Career clarity',
            ...
            label: 'General exploration',
            icon: Target
          }].map(p => <button key={p.id} type="button" onClick={() => set({
            ...s,
            purpose: p.id
        ...
        </div> : <p className="text-[13px] text-muted-foreground max-w-md leading-relaxed">{t("Onboarding.your_context_doesn_t")}</p>}
    </>,
  plan: (s, set) => {
    const plans = plansFor(s.persona, s.contextId);
    return <>
        <FieldHeading step={4} title={<>{t("Onboarding.choose_a")}<span className="text-muted-foreground">{t("Onboarding.starting_plan")}</span></>} />
        <p className="mb-8 max-w-xl text-[14px] leading-[1.75] text-muted-foreground">{t("Onboarding.the_core_assessment_is")}</p>
        <div className="grid md:grid-cols-2 gap-3">
          {plans.map(plan => {
          const active = s.plan === plan.id;
          const planDef = getPlan(plan.id);
          return <button key={plan.id} type="button" onClick={() => set({
            ...s,
            plan: plan.id
                ...
                <ul className="mt-4 space-y-2 flex-1">
                  {planDef.features.map(f => <li key={f} className="flex items-start gap-2 text-[12px] text-muted-foreground">
                      <span className="text-gold mt-0.5">·</span>{f}
                    </li>)}
                </ul>
                {planDef.status === 'proposed' && <span className="status-proposed border border-border px-2 py-1 mt-4 self-start text-[10px]">{t("Onboarding.in_development")}</span>}
              </button>;
        })}
        </div>
      </>;
  ...
  launch: s => {
    ...
    return <>
        <FieldHeading step={5} title={<>{t("Onboarding.you_re")}<span className="text-muted-foreground">{t("Onboarding.set")}</span></>} />
        <p className="mb-8 max-w-xl text-[14px] leading-[1.75] text-muted-foreground">{t("Onboarding.here_s_what_happens")}</p>
        <div className="grid md:grid-cols-2 gap-3">
          {[['Context', CONTEXTS.find(c => c.id === s.contextId)?.label || '—'], ['Role', s.persona || '—'], ['Goal', s.goalId || '—'], ['Age group', s.ageGroup || '—'], ['Plan', planDef.name], ['Destination', dest]].filter(([, v]) => v && v !== '—').map(([k, v]) => <div key={k} className="card p-5">
...
export default function Onboarding() {
  ...
  useEffect(() => {
    ...
    getOnboarding(user.uid).then(saved => {
      ...
      if (saved && !saved.completed) {
        ...
        if (saved.ageGroup && !saved.purpose) setStep(3);else setStep(firstEmpty === -1 ? 5 : firstEmpty);
      }
      setReady(true);
    });
    return () => {
  ...
  if (!user) {
    return null;
  } // ProtectedRoute handles redirect; brief guard
  if (!ready) {
    return <div className="min-h-screen bg-background text-on-surface flex items-center justify-center">
        <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-muted-foreground">{t("Onboarding.loading")}</span>
      </div>;
  }
  const update = next => {
    setState(next);
  ...
  const views = STEP_VIEWS(t);
  const render = views[STEPS[step].toLowerCase()] || views.context;
  return <div className="min-h-screen bg-background text-on-surface">
      <header className="flex min-h-20 items-center justify-between border-b border-border px-6 lg:px-12">
        <Wordmark to="/mode" />
        <button type="button" onClick={skip} disabled={saving} className="text-[11px] font-mono uppercase tracking-[0.15em] text-muted-foreground hover:text-on-surface no-underline transition-colors cursor-pointer bg-transparent border-none">{t("Onboarding.skip_for_now")}</button>
      </header>
      <main className="mx-auto max-w-[980px] px-6 py-10 lg:px-12 lg:py-14">
        ...
        <div className="mt-10 flex flex-wrap items-center justify-between gap-4 border-t border-border pt-6">
          <button type="button" onClick={() => setStep(Math.max(0, step - 1))} disabled={step === 0} className={`inline-flex items-center gap-2 text-[12px] font-mono uppercase tracking-[0.14em] ${step === 0 ? 'text-muted-foreground/40' : 'text-muted-foreground hover:text-on-surface'} cursor-pointer bg-transparent border-none disabled:cursor-not-allowed`}>
            <ArrowLeft size={14} />{t("Onboarding.back")}</button>
          <button type="button" onClick={handleNext} disabled={!canProceed || saving} className={`btn-primary no-underline ${!canProceed || saving ? 'opacity-40 cursor-not-allowed' : ''}`}>
