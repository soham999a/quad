...
export default function OnboardingGate({ children }) {
  ...
  const [state, setState] = useState({ loading: true, pass: false });

  useEffect(() => {
    let cancelled = false;
    if (!user) { setState({ loading: false, pass: false }); return; }
    setState({ loading: true, pass: false });
    (async () => {
      const saved = await getOnboarding(user.uid);
      if (cancelled) return;
      if (saved && saved.completed === true) { setState({ loading: false, pass: true }); return; }
      if (saved && !saved.completed) { setState({ loading: false, pass: false }); return; }
      // No onboarding doc: distinguish established users from brand-new ones.
      const activity = await getUserAssessments(user.uid);
      if (cancelled) return;
      setState({ loading: false, pass: Array.isArray(activity) && activity.length > 0 });
    })();
