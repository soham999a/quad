...
const SAMPLE_CLASSES = [
  { name: 'Grade 10 — Section A', gradeLevel: '10', subject: 'General Studies' },
  { name: 'Grade 11 — Science', gradeLevel: '11', subject: 'Science' },
];

const SAMPLE_STUDENTS = [
  'Emma Wilson', 'Liam Patel', 'Olivia Nguyen',
  'Noah Garcia', 'Ava Kim', 'Ethan Brown',
];

export default function SeedSchoolData({ onDone }) {
  ...
  const handleSeed = async () => {
    ...
    try {
      for (const cls of SAMPLE_CLASSES) {
        const { id: classId, classCode } = await createClass({
          name: cls.name,
  ...
  return (
    <div className="card p-8 text-center">
      ...
      <div className="text-label-md font-label-md text-on-background mb-2">No classes yet</div>
      <div className="text-body-sm font-body-sm text-surface-variant mb-6">
        Seed 2 sample classes with students to explore the teacher dashboard.
      </div>
