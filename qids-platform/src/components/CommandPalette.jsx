...
export default function CommandPalette({
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return items;
    return items.filter(it => it.label.toLowerCase().includes(q) || (it.keywords || '').toLowerCase().includes(q) || (it.group || '').toLowerCase().includes(q));
  }, [items, query]);
  ...
  let flatIndex = -1;
  return <div ref={modalRef} className="fixed inset-0 z-[120] flex items-start justify-center pt-[12vh] px-4" style={{
    background: 'color-mix(in oklab, var(--background) 72%, transparent)',
    backdropFilter: 'blur(8px)',
    WebkitBackdropFilter: 'blur(8px)'
  }} onMouseDown={e => {
    if (e.target === e.currentTarget) onClose();
  }} role="dialog" aria-modal="true" aria-label={t("CommandPalette.command_palette")}>
      <div className="w-full max-w-[560px] border border-border bg-surface-container-lowest animate-fade-up overflow-hidden" style={{
      boxShadow: '0 24px 80px color-mix(in oklab, var(--background) 60%, transparent)'
        ...
        <div className="flex items-center gap-3 px-5 h-14 border-b border-border">
          ...
          setActive(0);
        }} placeholder={t('palette.placeholder')} aria-label={t('palette.search_commands')} spellCheck="false" className="flex-1 bg-transparent border-none outline-none text-[14px] text-on-surface placeholder:text-outline font-technical-sm tracking-wide" />
          <kbd className="chip !py-0.5 !px-1.5 text-[9px]">{t("CommandPalette.esc")}</kbd>
        </div>

        {/* Results */}
        <div ref={listRef} className="max-h-[46vh] overflow-y-auto py-2" role="listbox">
          {filtered.length === 0 && <div className="px-5 py-8 text-center">
              <div className="section-index mb-2">{t('palette.no_matches')}</div>
              <p className="text-technical-sm font-technical-sm text-muted-foreground">
                {t('palette.no_matches_hint', {
              query
            })}
              </p>
            </div>}
          ...
          items: gItems
            ...
            return <button key={`${item.group}:${item.label}`} data-active={isActive} role="option" aria-selected={isActive} onMouseEnter={() => {
              // Map hover back to the flat index.
              const idx = filtered.indexOf(item);
              if (idx >= 0) setActive(idx);
            }} onClick={() => runItem(item)} className={`w-full flex items-center gap-3 px-5 py-2.5 text-left cursor-pointer border-none border-l-2 transition-colors ${isActive ? 'bg-surface-container-low text-on-surface border-[var(--gold)]' : 'bg-transparent text-muted-foreground border-transparent hover:text-on-surface'}`}>
        ...
        <div className="flex items-center gap-4 px-5 h-9 border-t border-border font-technical-sm text-[10px] tracking-[0.14em] uppercase text-muted-foreground">
          ...
          <span className="ml-auto">{t("CommandPalette.qids")}</span>
        </div>
      </div>
    </div>;
}