...
export default function ProcessNode({
  ...
  return <div style={{
    ...
    gap: 0
      ...
      <div onClick={() => onClick && onClick(node)} style={{
      ...
      padding: '0 4px'
        ...
        width: 20,
        height: 2,
        background: `linear-gradient(90deg, ${alpha(color, 80)}, ${alpha(color, 20)})`
      }} />
          <ChevronRight size={10} color={color} style={{
        opacity: 0.6
      }} />
        </div>}
    </div>;
  ...
  return <div style={{
    ...
    animation: 'fadeIn 0.2s ease'
      ...
      marginBottom: 20
        ...
        <button onClick={onClose} className="btn btn-ghost btn-sm" style={{
        padding: 6
      }}><X size={14} /></button>
      </div>
      ...
      {node.artifacts && <div>
        ...
        marginBottom: 8
          ...
          textTransform: 'uppercase',
          letterSpacing: '0.5px'
        }}>{t("ProcessNode.artifacts")}</span>
          </div>
          {node.artifacts.map(a => <div key={a} style={{
        padding: '6px 10px',
